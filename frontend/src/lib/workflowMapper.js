...
export function buildFinalSchema(schemaParams) {
  ...
  for (const parameter of Object.values(schemaParams)) {
    ...
    if (parameter.default !== undefined) {
      finalSchema[parameter.name.trim()].default = parameter.default;
    }
    if (parameter.example !== undefined) {
      finalSchema[parameter.name.trim()].example = parameter.example;
    }
    if (Array.isArray(parameter.choices) && parameter.choices.length) {
      finalSchema[parameter.name.trim()].choices = parameter.choices;
    }
  }
