export const messages = {
  en: {
    ...
    cloud_templates_title: "Cloud Example Workflows",
    cloud_templates_help: "Browse bundled runnable Cloud starters or discover official Comfy Cloud blueprints.",
    cloud_templates_source_bundled: "Bundled",
    cloud_templates_source_official: "Official",
    cloud_templates_empty: "No Cloud templates available.",
    cloud_templates_count: "{count} templates",
    cloud_templates_import: "Import",
    cloud_templates_imported: "Imported",
    cloud_templates_blueprint_only: "Blueprint only",
    cloud_templates_blueprint_note: "Discoverable Cloud blueprint. Direct runnable import is not available yet.",
    cloud_templates_runnable: "Runnable template. Import it into this Cloud server to expose it to the agent immediately.",
    cloud_templates_try_run: "Try run",
    cloud_templates_try_run_prompt: "Template \"{id}\" was imported. Do you want to try it now with the suggested starter args?",
    cloud_templates_import_success: "Cloud template \"{id}\" imported.",
    cloud_templates_run_success: "Cloud workflow finished. Downloaded {count} file(s).",
    cloud_templates_require_cloud_server: "Select a Comfy Cloud server before using Cloud examples.",
    err_load_cloud_templates: "Failed to load Cloud templates.",
    err_import_cloud_template: "Failed to import Cloud template.",
    err_run_cloud_template: "Failed to run imported Cloud workflow.",
    ...
    server_type_hint_cloud: "Use this for Comfy Cloud, Comfy's official hosted remote/cloud runtime. Self-hosted remote ComfyUI machines should usually stay on the regular ComfyUI type.",
    ...
    cloud_api_key_placeholder: "Paste API key",
    cloud_api_key_help: "Sent as the `X-API-Key` header. Leave blank to keep the existing stored key when editing, or use an environment variable instead.",
    ...
    cloud_api_key_env_label: "API Key Env Var",
    cloud_api_key_env_placeholder: "COMFY_CLOUD_API_KEY",
    cloud_api_key_env_help: "Recommended when you do not want to store the key in `config.json`.",
    ...
    cloud_templates_imported: "Imported",
    cloud_templates_blueprint_only: "Blueprint only",
    cloud_templates_runnable: "Runnable after import",
    cloud_templates_blueprint_note: "Discoverable blueprint. Only curated ones can be imported directly.",
    cloud_templates_import_success: "Imported Cloud workflow \"{id}\".",
    cloud_templates_try_run_prompt: "Run imported workflow \"{id}\" now with starter parameters?",
    cloud_templates_try_run: "Try Run",
    cloud_templates_run_success: "Cloud example finished. Downloaded {count} file(s).",
    cloud_templates_require_cloud_server: "Select a Comfy Cloud server to browse Cloud examples.",
    err_load_cloud_templates: "Failed to load Cloud templates.",
    err_import_cloud_template: "Failed to import Cloud template.",
    err_run_cloud_template: "Failed to run the imported Cloud workflow.",
    ...
    err_cloud_api_key_required: "Comfy Cloud requires an API key or API key env var.",
  ...
  zh: {
    ...
    cloud_templates_btn: "云端示例",
    cloud_templates_title: "云端示例工作流",
    cloud_templates_help: "浏览内置可运行的 Cloud starter，或查看官方 Comfy Cloud 蓝图模板。",
    cloud_templates_source_bundled: "内置",
    cloud_templates_source_official: "官方",
    cloud_templates_empty: "当前没有可用的 Cloud 模板。",
    cloud_templates_count: "{count} 个模板",
    cloud_templates_import: "导入",
    cloud_templates_imported: "已导入",
    cloud_templates_blueprint_only: "仅蓝图",
    cloud_templates_blueprint_note: "这是可发现的 Cloud 蓝图，暂时还不能直接导入为可运行工作流。",
    cloud_templates_runnable: "这是可运行模板。导入到当前 Cloud 服务器后，Agent 就能直接发现并调用。",
    cloud_templates_try_run: "立即试跑",
    cloud_templates_try_run_prompt: "模板 “{id}” 已导入。要不要用建议参数立即试跑一次？",
    cloud_templates_import_success: "Cloud 模板 “{id}” 已导入。",
    cloud_templates_run_success: "Cloud 工作流执行完成，已下载 {count} 个文件。",
    cloud_templates_require_cloud_server: "请先选择一个 Comfy Cloud 服务器，再使用云端示例。",
    err_load_cloud_templates: "加载 Cloud 模板失败。",
    err_import_cloud_template: "导入 Cloud 模板失败。",
    err_run_cloud_template: "运行导入后的 Cloud 工作流失败。",
    add_server: "添加服务器",
    server_name: "服务器名称",
    ...
    server_type_hint_cloud: "用于 Comfy Cloud，也就是 Comfy 官方提供的远程 / 云端服务器。客户端会通过 `X-API-Key` 鉴权，并走 Cloud 任务接口。你自己部署的远程 ComfyUI 机器通常仍应选择普通 ComfyUI 类型。",
    server_url_label: "服务器地址",
    server_url_help_comfyui: "直接调用标准 ComfyUI 端点：`/prompt`、`/history/{id}` 和 `/view`。",
    server_output_dir: "输出目录",
    cloud_base_url_label: "Cloud API 基础地址",
    cloud_base_url_placeholder: "https://cloud.comfy.org",
    cloud_api_key_label: "Cloud API Key",
    cloud_api_key_placeholder: "粘贴 API Key",
    cloud_api_key_help: "将作为 `X-API-Key` 请求头发送。编辑已有服务器时，留空可保留当前已存储的 key；如果你更希望从环境变量读取，也可以留空。",
    cloud_api_key_apply_tooltip_label: "如何申请 Cloud API Key",
    cloud_api_key_apply_tooltip: "前往 https://platform.comfy.org/login 登录，在 API Keys 中点击 + New，填写名称后点击 Generate。请立即保存，API Key 只会在创建时显示一次。",
    cloud_api_key_show: "显示 API Key",
    cloud_api_key_hide: "隐藏 API Key",
    cloud_api_key_env_label: "API Key 环境变量名",
    cloud_api_key_env_placeholder: "COMFY_CLOUD_API_KEY",
    cloud_api_key_env_help: "如果你不想把密钥写入 `config.json`，推荐使用这个方式。",
    cloud_partner_key_toggle: "同时把这个 API Key 写入 `extra_data.api_key_comfy_org`，供合作伙伴节点使用",
    cloud_call_scheme_help: "调用链路：`POST /api/prompt` -> `GET /api/job/{id}/status` -> `GET /api/history_v2/{id}` -> `GET /api/view`。",
    cloud_templates_btn: "云端示例",
    cloud_templates_title: "Cloud 示例工作流",
    cloud_templates_help: "浏览仓库内置的可运行 Cloud starter workflows，或查看官方 Comfy Cloud 蓝图。",
    cloud_templates_source_bundled: "内置模板",
    cloud_templates_source_official: "官方蓝图",
    cloud_templates_count: "共 {count} 个模板",
    cloud_templates_empty: "当前没有可用的 Cloud 模板。",
    cloud_templates_import: "导入",
    cloud_templates_imported: "已导入",
    cloud_templates_blueprint_only: "仅蓝图",
    cloud_templates_runnable: "导入后即可运行",
    cloud_templates_blueprint_note: "这是可发现的官方蓝图，只有经过整理的蓝图才支持直接导入。",
    cloud_templates_import_success: "Cloud 工作流「{id}」已导入。",
    cloud_templates_try_run_prompt: "是否立刻用默认参数试跑已导入的工作流「{id}」？",
    cloud_templates_try_run: "立即试跑",
    cloud_templates_run_success: "Cloud 示例已完成，下载了 {count} 个文件。",
    cloud_templates_require_cloud_server: "请先选择一个 Comfy Cloud 服务器，再浏览云端示例。",
    err_load_cloud_templates: "加载 Cloud 模板失败。",
    err_import_cloud_template: "导入 Cloud 模板失败。",
    err_run_cloud_template: "执行已导入的 Cloud 工作流失败。",
    no_servers: "暂无配置的服务器。",
    ...
    err_cloud_api_key_required: "Comfy Cloud 必须提供 API Key 或环境变量名。",
...
messages.zh_hant = {
  ...
  cloud_templates_btn: "雲端示例",
  cloud_templates_title: "雲端示例工作流",
  cloud_templates_help: "瀏覽內置可運行的 Cloud starter，或查看官方 Comfy Cloud 藍圖模板。",
  cloud_templates_source_bundled: "內置",
  cloud_templates_source_official: "官方",
  cloud_templates_empty: "目前沒有可用的 Cloud 模板。",
  cloud_templates_count: "{count} 個模板",
  cloud_templates_import: "導入",
  cloud_templates_imported: "已導入",
  cloud_templates_blueprint_only: "僅藍圖",
  cloud_templates_blueprint_note: "這是可發現的 Cloud 藍圖，暫時還不能直接導入為可運行工作流。",
  cloud_templates_runnable: "這是可運行模板。導入到目前 Cloud 伺服器後，Agent 就能直接發現並調用。",
  cloud_templates_try_run: "立即試跑",
  cloud_templates_try_run_prompt: "模板「{id}」已導入。要不要用建議參數立即試跑一次？",
  cloud_templates_import_success: "Cloud 模板「{id}」已導入。",
  cloud_templates_run_success: "Cloud 工作流執行完成，已下載 {count} 個文件。",
  cloud_templates_require_cloud_server: "請先選擇一個 Comfy Cloud 伺服器，再使用雲端示例。",
  err_load_cloud_templates: "加載 Cloud 模板失敗。",
  err_import_cloud_template: "導入 Cloud 模板失敗。",
  err_run_cloud_template: "運行導入後的 Cloud 工作流失敗。",
  add_server: "添加伺服器",
  server_name: "伺服器名稱",
  ...
  server_type_hint_cloud: "用於 Comfy Cloud，也就是 Comfy 官方提供的遠端 / 雲端伺服器。客戶端會透過 `X-API-Key` 驗證，並走 Cloud 任務介面。若是你自行部署的遠端 ComfyUI 機器，通常仍應選擇一般 ComfyUI 類型。",
  server_url_label: "伺服器地址",
  server_url_help_comfyui: "直接調用標準 ComfyUI 端點：`/prompt`、`/history/{id}` 與 `/view`。",
  server_output_dir: "輸出目錄",
  cloud_base_url_label: "Cloud API 基礎地址",
  cloud_base_url_placeholder: "https://cloud.comfy.org",
  cloud_api_key_label: "Cloud API Key",
  cloud_api_key_placeholder: "貼上 API Key",
  cloud_api_key_help: "會作為 `X-API-Key` 請求標頭送出。編輯既有伺服器時，留空可保留目前已儲存的 key；如果你希望從環境變數讀取，也可以留空。",
  cloud_api_key_apply_tooltip_label: "如何申請 Cloud API Key",
  cloud_api_key_apply_tooltip: "前往 https://platform.comfy.org/login 登入，在 API Keys 中點擊 + New，填寫名稱後按 Generate。請立即保存，API Key 只會在建立時顯示一次。",
  cloud_api_key_show: "顯示 API Key",
  cloud_api_key_hide: "隱藏 API Key",
  cloud_api_key_env_label: "API Key 環境變數名稱",
  cloud_api_key_env_placeholder: "COMFY_CLOUD_API_KEY",
  cloud_api_key_env_help: "如果你不想把密鑰寫入 `config.json`，建議使用這個方式。",
  cloud_partner_key_toggle: "同時把這個 API Key 寫入 `extra_data.api_key_comfy_org`，供合作夥伴節點使用",
  cloud_call_scheme_help: "調用鏈路：`POST /api/prompt` -> `GET /api/job/{id}/status` -> `GET /api/history_v2/{id}` -> `GET /api/view`。",
  cloud_templates_btn: "雲端示例",
  cloud_templates_title: "Cloud 示例工作流",
  cloud_templates_help: "瀏覽倉庫內建、可直接運行的 Cloud starter workflows，或查看官方 Comfy Cloud 藍圖。",
  cloud_templates_source_bundled: "內建模板",
  cloud_templates_source_official: "官方藍圖",
  cloud_templates_count: "共 {count} 個模板",
  cloud_templates_empty: "目前沒有可用的 Cloud 模板。",
  cloud_templates_import: "導入",
  cloud_templates_imported: "已導入",
  cloud_templates_blueprint_only: "僅藍圖",
  cloud_templates_runnable: "導入後即可運行",
  cloud_templates_blueprint_note: "這是可發現的官方藍圖，只有整理過的藍圖才支援直接導入。",
  cloud_templates_import_success: "Cloud 工作流「{id}」已導入。",
  cloud_templates_try_run_prompt: "是否立刻用預設參數試跑已導入的工作流「{id}」？",
  cloud_templates_try_run: "立即試跑",
  cloud_templates_run_success: "Cloud 示例已完成，下載了 {count} 個文件。",
  cloud_templates_require_cloud_server: "請先選擇一個 Comfy Cloud 伺服器，再瀏覽雲端示例。",
  err_load_cloud_templates: "載入 Cloud 模板失敗。",
  err_import_cloud_template: "導入 Cloud 模板失敗。",
  err_run_cloud_template: "執行已導入的 Cloud 工作流失敗。",
  no_servers: "暫無配置的伺服器。",
  ...
  err_cloud_api_key_required: "Comfy Cloud 必須提供 API Key 或環境變數名稱。",
